...
$Script:LogFileDisabled = $false
try {
  $logCandidates = @()
  if (-not [string]::IsNullOrWhiteSpace($PSScriptRoot)) { $logCandidates += $PSScriptRoot }
  if (-not [string]::IsNullOrWhiteSpace($PSCommandPath)) { $logCandidates += (Split-Path -Path $PSCommandPath -Parent) }
  if ($MyInvocation -and $MyInvocation.MyCommand -and $MyInvocation.MyCommand.Path) {
    $logCandidates += (Split-Path -Path $MyInvocation.MyCommand.Path -Parent)
  }
  try {
    $pwdCandidate = (Get-Location).ProviderPath
    if (-not [string]::IsNullOrWhiteSpace($pwdCandidate)) { $logCandidates += $pwdCandidate }
  }
  catch {}
  foreach ($candidate in $logCandidates) {
    ...
    try {
      if (Test-Path -LiteralPath $candidate) {
        $Script:LogFilePath = Join-Path -Path $candidate -ChildPath 'AD-BulkUserCreator.log'
        break
      }
    }
    catch {}
  }
  if (-not $Script:LogFilePath) { $Script:LogFileDisabled = $true }
}
catch {
  $Script:LogFilePath = $null
  $Script:LogFileDisabled = $true
}
...
function Write-LogFile {
  ...
  if ($Script:LogFileDisabled -or -not $Script:LogFilePath) { return }
  $level = $Type.ToUpperInvariant()
  $entry = '{0:yyyy-MM-dd HH:mm:ss} [{1}] {2}' -f $Timestamp, $level, $Text
  try {
    Add-Content -LiteralPath $Script:LogFilePath -Value $entry -Encoding UTF8 -ErrorAction Stop
  }
  catch {
    $Script:LogFileDisabled = $true
  }
...
function Write-ToTextBox {
  ...
  $entry = '{0:yyyy-MM-dd HH:mm:ss} {1} {2}' -f $timestamp, $prefix, $Text
  if ($tb_logg_box) {
  ...
  Write-LogFile -Text $Text -Type $Type -Timestamp $timestamp
}

$Script:DuplicateDebug = $true
function Write-DuplicateDebug {
  param([string]$Text)
  if (-not $Script:DuplicateDebug) { return }
  Write-ToTextBox ("[DEBUG] {0}" -f $Text) 'Info'
}
...
function Add-DuplicateAlertEntry {
  param(
  ...
  if (-not $List -or [string]::IsNullOrWhiteSpace($Login)) { return }
  $label = if ([string]::IsNullOrWhiteSpace($DisplayLabel)) { $Login } else { $DisplayLabel }
  $reasonText = if ($Reasons) {
    $clean = $Reasons | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }
    if ($clean -and $clean.Count -gt 0) { ($clean | Select-Object -Unique) -join ', ' } else { $null }
  }
  else { $null }
  if ([string]::IsNullOrWhiteSpace($reasonText)) { $reasonText = 'nieznany konflikt' }
  $null = $List.Add(("{0} (login: {1}) - konflikt: {2}" -f $label, $Login, $reasonText))
}

function Get-DuplicateReasonsFromError {
  param(
    $ErrorRecord,
    [string]$FriendlyHint
  )
  ...
  if (-not [string]::IsNullOrWhiteSpace($FriendlyHint) -and ($FriendlyHint -match '(?i)istnieje|exists|duplicate|in use|zaj[ęe]ty')) {
    $null = $reasons.Add($FriendlyHint)
  }
  $raw = $null
  try { $raw = [string]$ErrorRecord.Exception.Message } catch { $raw = [string]$ErrorRecord }
  if (-not [string]::IsNullOrWhiteSpace($raw) -and ($raw -match '(?i)already|exists|duplicate|in use|istnieje|zaj[ęe]ty|kolizja')) {
    $null = $reasons.Add($raw)
  }
  ...
  try {
    ...
    if ($selectedTab -eq 'Student') {
      for ($r = 0; $r -lt $grid.Rows.Count; $r++) {
        ...
        $album = [string]$row.Cells[$ColNrAlbumu].Value
...
foreach ($name in $tabNames) {
  ...
  switch ($name) {
    'Uczen'   { $columns = @($ColLp, $ColImie, $ColNazwisko, $ColNazwaWyswietlana, $ColLogin, $ColMiasto, $ColEmail, $ColHaslo) }
    'Student' { $columns = @($ColLp, $ColImie, $ColNazwisko, $ColNrAlbumu, $ColNazwaWyswietlana, $ColLogin, $ColMiasto, $ColEmail, $ColHaslo) }
    default   { $columns = @($ColLp, $ColImie, $ColNazwisko, $ColNazwaWyswietlana, $ColLogin, $ColMiasto, $ColEmail, $ColHaslo) }
  }
  ...
  try { if ($toolTip) { $toolTip.SetToolTip($pasteBtn, 'Wkleja dane z TAB ze schowka') } } catch {}
  $pasteBtn.Add_Click({
      try {
        $selectedTab = $tabs.SelectedTab.Text
...
"
        ...
        foreach ($line in $linesText) {
          ...
          if ($selectedTab -eq 'Student' -and $vals.Count -ge 3) { $row.Cells[$ColNrAlbumu].Value = ([string]$vals[2]).Trim() }
        }
  ...
  try { if ($toolTip) { $toolTip.SetToolTip($checkBtn, 'Sprawdza dane i kolizje w AD') } } catch {}
  ...
  try { if ($toolTip) { $toolTip.SetToolTip($refreshBtn, 'Przelicza loginy, e-mail i domyslne pola') } } catch {}
  ...
  try { if ($toolTip) { $toolTip.SetToolTip($btnDeleteRow, 'Usuwa zaznaczony wiersz') } } catch {}
  ...
  # Kopiuj dane (TSV: Email, Nazwa wyswietlana, Haslo) – przeniesione do wiersza z Wklej/Usun/WhatIf
  $btnCopyTab = New-Object System.Windows.Forms.Button
  $btnCopyTab.Text = 'Kopiuj'
  & $styleButton $btnCopyTab
  try { if ($toolTip) { $toolTip.SetToolTip($btnCopyTab, 'Kopiuje TSV: Email, Nazwa wyswietlana, Haslo') } } catch {}
  $btnCopyTab.Margin = [System.Windows.Forms.Padding]::new(0, 0, 8, 0)
  try { Set-ButtonStockIcon -Button $btnCopyTab -Siid 1 -Small } catch {}
  $btnCopyTab.Add_Click({
      try {
        $selectedTab = $tabs.SelectedTab.Text
        $grid = $grids[$selectedTab]
        if (-not $grid) { Write-ToTextBox "Brak gridu dla zakladki $selectedTab" 'Warning'; return }
        ...
        # Kopiuj tylko wiersze, dla ktorych konto istnieje w AD.
        # Jesli modul AD niedostepny lub sprawdzenie sie nie powiedzie, kopiuj wszystko.
        $adAvailable = $true
        try { if (-not (Test-ADModule)) { $adAvailable = $false } } catch { $adAvailable = $false }
        ...
        for ($r = 0; $r -lt $grid.Rows.Count; $r++) {
          ...
          $display = [string]$row.Cells[$ColNazwaWyswietlana].Value
          $pass = [string]$row.Cells[$ColHaslo].Value

          $include = $true
          if ($adAvailable) {
            try {
              $u = Get-ADUser -LDAPFilter "(sAMAccountName=$login)" -ErrorAction Stop
              $include = [bool]$u
            }
            catch {
              # AD check failed mid-way; fallback to include all rows
              $adAvailable = $false
              $include = $true
            }
          }
          # Wyznacz e-mail z kolumny lub zloz go z loginu/albumu i domeny z GUI
          $domainForAccount = if ($cbDomain.SelectedItem) { [string]$cbDomain.SelectedItem } else { $Domain_Defaults[$selectedTab] }
          $email = [string]$row.Cells[$ColEmail].Value
          if ([string]::IsNullOrWhiteSpace($email)) {
          $loginForEmail = if ($selectedTab -eq 'Student' -and $row.Cells[$ColNrAlbumu].Value) { ([string]$row.Cells[$ColNrAlbumu].Value).Trim() } else { $login }
            if ([string]::IsNullOrWhiteSpace($loginForEmail)) { continue }
            $email = ("{0}@{1}" -f $loginForEmail, $domainForAccount)
          }
          if ($include) {
            $lines.Add(("{0}`t{1}`t{2}" -f $email, $display, $pass)) | Out-Null
          }
        }

        if ($lines.Count -eq 0) {
          Write-ToTextBox "Brak danych do skopiowania (sprawdz, czy konta zostaly utworzone)." 'Warning'
          return
        }
        $text = ($lines -join [Environment]::NewLine)
        Set-Clipboard -Value $text
        Write-ToTextBox ("Skopiowano {0} wierszy do schowka (zakladka {1})." -f $lines.Count, $selectedTab) 'Success'
      }
      catch {
        Write-ToTextBox "Blad podczas kopiowania danych do schowka: $_" 'Error'
      }
    })
  $buttonsLeft.Controls.Add($btnCopyTab) | Out-Null

  # Eksport do CSV – przeniesiony do wiersza z Wklej/Usun/WhatIf
  $btnExportTab = New-Object System.Windows.Forms.Button
  $btnExportTab.Text = 'Eksport CSV'
  & $styleButton $btnExportTab
  try { if ($toolTip) { $toolTip.SetToolTip($btnExportTab, 'Eksportuje biezaca zakladke do CSV') } } catch {}
  $btnExportTab.Margin = [System.Windows.Forms.Padding]::new(0, 0, 8, 0)
  try { Set-ButtonStockIcon -Button $btnExportTab -Siid 1 -Small } catch {}
  ...
  $buttonsLeft.Controls.Add($btnExportTab) | Out-Null

  # Wyczyść zakladkę – przeniesiony do wiersza z Wklej/Usun/WhatIf
  $btnClearTab = New-Object System.Windows.Forms.Button
  $btnClearTab.Text = 'Wyczyść'
  & $styleButton $btnClearTab
  try { if ($toolTip) { $toolTip.SetToolTip($btnClearTab, 'Czysci wszystkie wiersze w zakladce') } } catch {}
  try { Set-ButtonStockIcon -Button $btnClearTab -Siid 31 -Small } catch {}
  $btnClearTab.Add_Click({
      $selectedTab = $tabs.SelectedTab.Text
      $grids[$selectedTab].Rows.Clear()
      Write-ToTextBox "Wyczyszczono dane z zakladki $selectedTab." 'Info'
    })
  $buttonsLeft.Controls.Add($btnClearTab) | Out-Null

  $btnCreate = New-Object System.Windows.Forms.Button
  $btnCreate.Text = 'Utwórz'
  & $styleButton $btnCreate -Primary
  $btnCreate.Add_Click({
      try {
        $selectedTab = $tabs.SelectedTab.Text
        $grid = $grids[$selectedTab]
        ...
        if (@('Student', 'Pracownik') -contains $selectedTab) {
          $duplicateAlerts = New-Object System.Collections.Generic.List[string]
        }
        ...
        foreach ($row in $grid.Rows) {
          ...
          try {
            ...
            $conflictReasons = @()
            if ($conf.LoginExists) { Write-ToTextBox "Login juz istnieje w AD: $login - zmien login (wiersz: $($row.Index))" 'Error'; $conflictReasons += 'login' }
            if ($conf.NameExists) { Write-ToTextBox "Nazwa (CN/Name) w OU juz istnieje: '$cn' - zmien format nazwy lub wybierz inne OU (wiersz: $($row.Index))" 'Error'; $conflictReasons += 'nazwa' }
            if ($conflictReasons.Count -gt 0) {
              if ($duplicateAlerts) {
                $displayLabel = if (-not [string]::IsNullOrWhiteSpace($cn)) { $cn } else { "$imi $naz" }
                Add-DuplicateAlertEntry -List $duplicateAlerts -DisplayLabel $displayLabel -Login $login -Reasons $conflictReasons
                Write-DuplicateDebug ("Pre-check duplicate captured for {0} (login: {1}) reasons: {2}" -f $displayLabel, $login, ($conflictReasons -join ', '))
              }
              continue
            }
            $params = @{
            ...
            if ($WhatIf_ByTab.ContainsKey($selectedTab) -and $WhatIf_ByTab[$selectedTab].Checked) {
              ...
              try {
                New-ADUser @params; $createdCount++
                Write-ToTextBox "Utworzono konto: $login ($dn)" 'Success'
              }
              catch {
                $errRecord = $_
                $hint = Get-FriendlyADError -ErrorRecord $errRecord
                Write-ToTextBox ("Blad tworzenia konta {0}: {1}" -f $login, $hint) 'Error'
                if ($duplicateAlerts -and $isStrictDuplicateTab) {
                  $dupReasons = Get-DuplicateReasonsFromError -ErrorRecord $errRecord -FriendlyHint $hint
                  if ($dupReasons) {
                    $displayLabel = if (-not [string]::IsNullOrWhiteSpace($cn)) { $cn } else { "$imi $naz" }
                    Add-DuplicateAlertEntry -List $duplicateAlerts -DisplayLabel $displayLabel -Login $login -Reasons $dupReasons
                    Write-DuplicateDebug ("AD error duplicate captured for {0} (login: {1}) reasons: {2}" -f $displayLabel, $login, ($dupReasons -join ', '))
                  }
                }
                continue
              }
            ...
            Write-ToTextBox "Blad tworzenia konta $($login): $_" 'Error'
            if ($duplicateAlerts -and $isStrictDuplicateTab) {
              $dupReasonsOuter = Get-DuplicateReasonsFromError -ErrorRecord $_ -FriendlyHint $null
              if ($dupReasonsOuter) {
                $displayLabelOuter = if (-not [string]::IsNullOrWhiteSpace($cn)) { $cn } else { "$imi $naz" }
                Add-DuplicateAlertEntry -List $duplicateAlerts -DisplayLabel $displayLabelOuter -Login $login -Reasons $dupReasonsOuter
                Write-DuplicateDebug ("Outer error duplicate captured for {0} (login: {1}) reasons: {2}" -f $displayLabelOuter, $login, ($dupReasonsOuter -join ', '))
              }
            }
          }
        }
        if ($duplicateAlerts -and $duplicateAlerts.Count -gt 0) {
          Write-DuplicateDebug ("duplicateAlerts entries: {0}" -f $duplicateAlerts.Count)
          $body = "Nastepujace konta juz istnieja w AD:`r`n - " + ($duplicateAlerts -join "`r`n - ")
          try {
            [System.Windows.Forms.MessageBox]::Show($body, 'Konta juz istnieja', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning) | Out-Null
          }
          catch {}
        }
        else {
          Write-DuplicateDebug 'duplicateAlerts empty — dialog skipped'
        }
        # Uruchom synchronizacje AAD Connect (Delta) po utworzeniu kont
        $whatIfActive = ($WhatIf_ByTab.ContainsKey($selectedTab) -and $WhatIf_ByTab[$selectedTab].Checked)
        if (-not $whatIfActive -and $createdCount -gt 0) {
          try {
            Start-ADSyncSyncCycle -PolicyType Delta | Out-Null
            Write-ToTextBox 'Wystartowano synchronizacje AAD Connect (Delta).' 'Info'
          }
          catch {
            Write-ToTextBox 'Nie udalo sie uruchomic Start-ADSyncSyncCycle -PolicyType Delta.' 'Warning'
          }
        }
      }
      catch {
  ...
  try {
    $btnCreate.Margin = [System.Windows.Forms.Padding]::new(0, 0, 8, 0)
    if ($checkBtn -and $checkBtn.Height -gt 0) { $btnCreate.Height = $checkBtn.Height }
  }
  catch {}
...
# Kopiuj dane do TSV (bez naglowka) – obok eksportu CSV
$btnCopyTsv = New-Object System.Windows.Forms.Button
$btnCopyTsv.Text = 'Kopiuj dane'
& $styleButton $btnCopyTsv
try { Set-ButtonStockIcon -Button $btnCopyTsv -Siid 1 -Small } catch {}
$btnCopyTsv.Add_Click({
    try {
      $selectedTab = $tabs.SelectedTab.Text
      $grid = $grids[$selectedTab]
      if (-not $grid) { Write-ToTextBox "Brak gridu dla zakladki $selectedTab" 'Warning'; return }
...
$null = $null # przeniesiono przyciski Export/Kopiuj/Wyczysc do wiersza zakladki
